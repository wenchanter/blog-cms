...
一个约定，不需要额外操作：**引用块最后一段以 `—` 开头会成为署名**。

> 曾经还有一条"正文第一段自动成为 lead（更大更深的导语样式）"，**已经移除**。它的本意是让排版免费发生，但导语样式只在开场是一两句话时成立；开头写长一点，整段被放大加深就像格式坏了，于是作者反而要记着"第一段不能写长"——正是这个编辑器要消除的那种打断。
>
> `lead` 仍留在块模型里，两侧渲染器也都还支持，历史文章不受影响；只是编辑器不再产出新的。重新编辑并保存一篇老文章，它的 lead 会降级为普通段落，这是单向的、有意的。
